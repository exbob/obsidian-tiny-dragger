...
悬停一块。左侧 gutter 上会出现四点手柄（叠在 gutter 上，不会挤窄正文）：

- **点击**四点打开块菜单。菜单关闭前，手柄会一直留在该块上。
- **按住拖拽**四点可移动该块。
...
- **列表：** 每一行列表单独移动。未选中的嵌套子项不会一起带走；若选中多行嵌套项（例如三个缩进子项），则可一起拖走。
- **嵌套：** 拖拽列表、引用或 callout 时，横向移动可改缩进；辅助线会按目标缩进水平偏移。基本竖直拖拽则保持原缩进。
- 按 **Escape** 取消。

### 块菜单

点击四点（不要拖拽）可转换、复制、剪切或删除。转换目标为段落、H1–H6、无序 / 有序 / 任务列表、引用、`[!info]` / `[!warning]` / `[!danger]` / `[!example]`、代码块或数学公式块；转换项与复制 / 剪切 / 删除之间有一条分割线。同类型转换无效果。当原生多块选区包含被点中的手柄块时，这些操作作用于整组载荷（规则与拖拽相同）；否则只作用于被点中的那一块。
